...
- **Mission Control UI:** Next.js 16 app (`apps/ui`) that surfaces workflow tracking, actions, chat, terminal, and docs. Intended to be wrapped in Electron for desktop parity and adapted for VS Code with Copilot-backed chat (no embedded terminal/BYOK there).
...
- **Repository registry:** `apps/ui/lib/repository-store.ts` stores known repos, active repo, and BMAD install/initialization flags.
