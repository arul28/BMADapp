...
- Node.js 20+
- npm (root CLI/tooling) and pnpm (UI, pnpm-lock.yaml in `apps/ui`)
...
- UI: `apps/ui` (Next.js), with components under `components/` and data/stores under `lib/`.
- Excluded from scans: `.bmad/` (installer payload), `devDocs/` (outputs).

## Electron/VS Code Notes

- Electron: wrap the `apps/ui` UI; map `terminal-manager` to embedded terminal; keep BYOK chat.
