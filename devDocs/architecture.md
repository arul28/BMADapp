...
- Node 22 (`.nvmrc`), npm (root) + pnpm (apps/ui).
...
- Data ingestion: preload (desktop) or VS Code FS API reads `devDocs/bmm-workflow-status.yaml` (including `selected_track`, `field_type`, and `workflow_path`), docScan outputs, agent/workflow definitions; parsed via yaml/markdown to feed board/docs views. When `workflow_path` is present, load the referenced YAML from `.bmad/bmm/workflows/workflow-status/paths/*` and map its phases/workflows/flags (required/optional/recommended/conditional/prerequisite) into the board schema. If `workflow_path` is missing, surface an actionable error and allow manual selection of track + field_type (quick-flow, bmad-method, enterprise; greenfield/brownfield) before retrying.
...
- Feature-first structure in `apps/ui/components` (board, chat, terminal, docs) with shared primitives under `components/ui`; shared logic under `lib`.
...
- Assets/config: keep static assets under `apps/ui/public`; config under repo root (`.nvmrc`, `package.json`, `eslint.config.mjs`); Electron/extension configs under `apps/desktop` and `apps/vscode-extension`.
- IPC definitions under `apps/desktop/preload/ipc.ts` (to add) and mirrored TS types under `apps/ui/lib/ipc-types.ts`.
...
- App Router routes live in `apps/ui/app`; domain components in `apps/ui/components`; shared logic in `apps/ui/lib`.
- Electron-specific code in `apps/desktop` (main, preload, builder config) isolated from UI bundle.
- VS Code extension hosts webview bundle from `apps/ui/.next` export; extension activation/context kept in `apps/vscode-extension`.
...
- Desktop: electron-builder 24.x targeting macOS (dmg/zip); bundle Next build (`pnpm build` in apps/ui); start `next start` on localhost with randomized port; preload locks repo scope.
...
- Node 22 (per `.nvmrc`), npm 10+, pnpm 9+ for apps/ui.
